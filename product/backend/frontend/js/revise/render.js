...
function render() {
  ...
  state.data.walls.forEach(w => {
    ...
    if (!nodes[sx+','+sy]) nodes[sx+','+sy] = [];
    nodes[sx+','+sy].push({ wall: w, isStart: true });
    
    if (!nodes[ex+','+ey]) nodes[ex+','+ey] = [];
    nodes[ex+','+ey].push({ wall: w, isStart: false });
  });
  ...
  Object.values(nodes).forEach(list => {
    if (list.length === 2) {
      ...
      const isH1 = wallIsHorizontal(w1), isH2 = wallIsHorizontal(w2);
      if (isH1 !== isH2) {
        const hw = isH1 ? w1 : w2;
        const vw = isH1 ? w2 : w1;
        ...
        const th = Math.max(hw.thickness, 2) * state.scale;
        const tv = Math.max(vw.thickness, 2) * state.scale;
        const sc = ws(nodeX, nodeY);
        
        fillerRects.push({
          x: gapDirX > 0 ? sc.x : sc.x - tv/2,
          y: gapDirY > 0 ? sc.y : sc.y - th/2,
          w: tv/2,
          h: th/2
        });
      }
  ...
  // ── 4. Windows, Doors, and Selection Outlines ─────────────────────────────
  state.data.walls.forEach(wall => {
