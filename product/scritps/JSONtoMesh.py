...
def create_floorplan(json_path, scale, wall_height, door_height, window_height, window_sill, invert_y, img_h)
    ...
    for wall_data in walls
        ...
        for i, door in enumerate(wall_data.get(doors, []))
            pos = door[snapped_at]
            bbox = door.get(original_bbox, {})
            
            d_width = max(bbox.get(xmax, 0) - bbox.get(xmin, 0), bbox.get(ymax, 0) - bbox.get(ymin, 0))
            if d_width == 0 d_width = 20.0 
            
            wx, wy = pos[x], pos[y]
            if invert_y wy = img_h - wy
            ...
            bool_mod = wall_obj.modifiers.new(name=fDoor_{i}, type='BOOLEAN')
        ...
        for i, window in enumerate(wall_data.get(windows, []))
            pos = window[snapped_at]
            bbox = window.get(original_bbox, {})
            
            w_width = max(bbox.get(xmax, 0) - bbox.get(xmin, 0), bbox.get(ymax, 0) - bbox.get(ymin, 0))
            if w_width == 0 w_width = 30.0 
            ...
            bool_mod = wall_obj.modifiers.new(name=fWindow_{i}, type='BOOLEAN')
            bool_mod.operation = 'DIFFERENCE'
...
class OBJECT_OT_generate_floorplan(bpy.types.Operator)
    ...
    def execute(self, context)
        ...
        try
            ...
            self.report({'ERROR'}, fFailed to build layout {str(e)})
            return {'CANCELLED'}
...
# Add directly into Blender's structural Add Menu top header dropdown
